...
type 1          type 2
26z+w+X         z // 26 or 26(z//26+w+Y)

type 1 increases z about 26 times
type 2 decreases z about 26 times if we have the left one of type two and stays roughly the same of we have the right one
7 increases and 7 decreases
we need the round down! as the other increases every time! so the left one of type 2
how do we guarantee a decrease wel that happens in type 2 calculations and only if
the (z%26)-X == w

So to be clear:
- z is maintained throughout the whole run
- every digit modifies z
- type 1: z = 26z + w + X?
- type 2: z = z // 26
...
type_1 = [13, 10, 5, None, 5, None,4, 11, 1, None, None, None, None, None]
type_2 = [None, None, None, 11, None, 0, None, None, None, 6, 10, 12, 3, 5]


def correct(digits: list[int]):
    z = 0
    res = [0] * 14
    idx = 0
...
def part_1(source):
    ...
    print(len(source))
    for i in range(14):
        # W[i] = ints(source[i * 18 ])[0]
        Y[i] = ints(source[i * 18 + ADD_Y_POS])[0]
        X[i] = ints(source[i * 18 + ADD_X_POS])[0]
...
class UnitTests(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.test_source = read_rows(f"day_{day.zfill(2)}_t1.input")
